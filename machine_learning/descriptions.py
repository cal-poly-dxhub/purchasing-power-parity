...
def get_data(query, category):
    ...
    awsURL = "https://5e7hnb1vb3.execute-api.us-west-2.amazonaws.com/dev/dynamo/v2?"
    # what I am querying
    query_qs = 'product={}'.format(query)
...
def grab_product_descriptions(frames, headers):
    ...
    for frame in frames:
        ...
        if 'uuid' in frame['frame']:
            uuids = frame['frame']['uuid']
        descriptions[store] = {'products': values, 'uuids': uuids}
    return descriptions
    ...
    if len(descriptions) > 0:
        ...
        for key in descriptions:
            formatted_descriptions[query].append(
                {key: {'products': descriptions[key]['products'].values, 'uuids': descriptions[key]['uuids'].values}})
