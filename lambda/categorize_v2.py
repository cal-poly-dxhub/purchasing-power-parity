    ...
    for product_array in products[query]:  # [query]:
        for store in product_array:
            for index in range(len(product_array[store]['products'])):
                ...
                doc_array['uuids'].append(uuid4()) # CHANGE TO ACTUAL UUID
...
TODO: CHange the function below to handle product UUID, when it is implemented.
"""
            ...
            for index in range(len(products['products'])):
                ...
                sim_score = [
                    {'product': str(products['products'][index]),
                     ...
                     'category': category,
                     'uuid': uuid4()} for category in categories] # CHANGE TO ACTUAL UUID
...
def generate_categories(nlp, matcher, products, query, category):
    ...
    for key in categories:
        print("\nPrinting products matching {},{}".format(query, key))
...
def insert_products_into_database(matched_products, generated_categories):
...
def insert_subcategories_into_database(generated_categories, parent_code):
    insert_stmt = """INSERT INTO subcategories (Name, Parent_Id) VALUES"""
    stmt_array = []
...
if __name__ == "__main__":
    ...
    insert_statements = insert_subcategories_into_database(generated_categories, parent_code)
